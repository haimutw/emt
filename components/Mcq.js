  ...
  template: `
    <div class="bg-slate-50 dark:bg-slate-900 md:rounded-2xl shadow-sm border-x border-b md:border-t border-slate-200 dark:border-slate-700 relative overflow-hidden flex flex-col w-full h-full transition-colors">
      ...
      <div v-if="!currentMcqFile" class="p-5 md:p-10 flex-1 flex flex-col">
        <h2 class="text-2xl md:text-3xl font-bold text-medical-blue dark:text-blue-400 mb-6 md:mb-8 border-l-4 border-medical-blue dark:border-blue-400 pl-4 tracking-wide">選擇測驗題組</h2>
        <div class="grid gap-4 md:gap-6 sm:grid-cols-2 lg:grid-cols-3">
          
          <button v-for="item in catalogs.questions" :key="item.file" @click="openMcq(item.file)" class="text-left p-6 md:p-8 rounded-3xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-medical-blue dark:hover:border-blue-500 hover:shadow-xl transition-all group flex flex-col justify-between h-full relative overflow-hidden">
            ...
            <div>
              <i class="fa-solid fa-file-signature text-4xl md:text-5xl text-slate-300 dark:text-slate-600 group-hover:text-medical-blue dark:group-hover:text-blue-400 mb-4 md:mb-6 block transition-colors mt-2"></i>
              <span class="font-black text-lg md:text-xl text-slate-700 dark:text-slate-200 group-hover:text-medical-blue dark:group-hover:text-blue-400 leading-snug">{{ item.title }}</span>
            </div>
            <div class="mt-6 flex justify-end">
              <span class="text-xs md:text-sm font-bold text-slate-400 dark:text-slate-500 group-hover:text-medical-blue dark:group-hover:text-blue-400 bg-slate-50 dark:bg-slate-700 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/30 px-4 py-2 rounded-lg transition-colors border border-slate-100 dark:border-slate-600 shadow-sm">開始測驗 <i class="fa-solid fa-arrow-right ml-1"></i></span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div v-else class="flex flex-col flex-1 h-full relative">
        <div class="sticky top-0 z-20 bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 p-3 md:p-5 flex justify-between items-center shadow-sm shrink-0 transition-colors">
          <div class="flex items-center gap-3 md:gap-4">
            <button @click="goBack" class="text-slate-500 dark:text-slate-400 hover:text-medical-blue dark:hover:text-blue-400 w-10 h-10 md:w-auto md:px-5 md:py-2.5 rounded-full bg-slate-100 dark:bg-slate-700 hover:bg-blue-50 dark:hover:bg-slate-600 transition-colors flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-600 shadow-sm font-bold text-sm md:text-base">
              <i class="fa-solid fa-arrow-left"></i> <span class="hidden md:block">返回題庫</span>
            </button>
            <div v-if="!showExplanations" class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 text-medical-blue dark:text-blue-400 px-3 py-1 md:px-4 md:py-1.5 rounded-lg text-xs md:text-sm font-bold shadow-sm">
              <i class="fa-solid fa-list-ol mr-1"></i> 共 {{ mcqData.length }} 題 / 每題 {{ Math.round((100 / mcqData.length) * 10) / 10 }} 分
            </div>
          </div>
          <div v-if="scoreResult !== null" class="bg-slate-800 dark:bg-slate-900 border dark:border-slate-700 text-white px-3 md:px-6 py-1.5 md:py-2 rounded-xl text-sm md:text-lg font-black shadow-inner flex items-center gap-2">
            <i class="fa-solid fa-star text-yellow-400"></i>
            得分: <span :class="scoreResult >= 80 ? 'text-green-400' : 'text-red-400'">{{ scoreResult }}</span>
          </div>
        </div>
        
        <div ref="mcqContainer" class="p-4 md:p-10 flex-1 overflow-y-auto overscroll-none no-scrollbar w-full flex flex-col items-center bg-slate-100 dark:bg-slate-900 transition-colors">
          <div class="max-w-4xl w-full space-y-6 md:space-y-8 pb-10">
            ...
            <div v-for="(q, index) in mcqData" :key="q.id" class="bg-white dark:bg-slate-800 p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border-2 border-slate-200 dark:border-slate-700 relative overflow-hidden transition-all hover:shadow-md">
              <div class="absolute top-0 left-0 bg-medical-blue dark:bg-blue-700 text-white px-5 py-1.5 rounded-br-2xl font-mono text-sm font-bold shadow-sm">Q{{ index + 1 }}</div>
              <p class="font-black text-lg md:text-2xl text-slate-800 dark:text-slate-100 mb-6 mt-5 leading-relaxed tracking-wide">{{ q.question }}</p>
              <div class="space-y-3 md:space-y-4">
                <button v-for="(opt, optIndex) in q.options" :key="optIndex" @click="selectAnswer(index, optIndex)" :disabled="showExplanations" :class="['w-full text-left p-4 md:p-5 rounded-2xl border-2 transition-all flex items-center gap-3 md:gap-5 group outline-none', showExplanations ? 'cursor-not-allowed' : 'hover:border-medical-blue dark:hover:border-blue-500 active:scale-[0.98]', userAnswers[index] === optIndex ? 'border-medical-blue dark:border-blue-500 bg-blue-50/50 dark:bg-blue-900/30 shadow-sm' : 'border-slate-100 dark:border-slate-600 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600']">
                  <div :class="['w-6 h-6 md:w-7 md:h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors', userAnswers[index] === optIndex ? 'border-medical-blue dark:border-blue-400 bg-white dark:bg-slate-800' : 'border-slate-300 dark:border-slate-500 group-hover:border-medical-blue dark:group-hover:border-blue-400 bg-white dark:bg-slate-800']"><div v-if="userAnswers[index] === optIndex" class="w-3 h-3 md:w-3.5 md:h-3.5 bg-medical-blue dark:bg-blue-400 rounded-full"></div></div>
                  <span :class="['text-base md:text-lg font-bold leading-snug', userAnswers[index] === optIndex ? 'text-medical-blue dark:text-blue-400' : 'text-slate-600 dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-slate-100']">{{ opt }}</span>
              ...
              <transition name="fade-slide">
                <div v-if="showExplanations" class="mt-6 md:mt-8 overflow-hidden rounded-2xl border-2 shadow-inner" :class="userAnswers[index] === q.answer ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'">
                  <div class="p-5 md:p-6 flex flex-col md:flex-row items-start gap-4 md:gap-6">
                    <div :class="['shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center text-white font-black text-2xl md:text-3xl shadow-md', userAnswers[index] === q.answer ? 'bg-green-500 dark:bg-green-600' : 'bg-red-500 dark:bg-red-600']">
                      <i :class="['fa-solid', userAnswers[index] === q.answer ? 'fa-check' : 'fa-xmark']"></i>
                    </div>
                    <div class="flex-1 w-full">
                      <h4 :class="['font-black text-lg md:text-xl mb-1 md:mb-2 tracking-wide', userAnswers[index] === q.answer ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400']">{{ userAnswers[index] === q.answer ? '回答正確' : '回答錯誤' }}</h4>
                      <p class="text-sm md:text-base font-bold text-slate-600 dark:text-slate-300 leading-relaxed mb-4">正確答案為：<span class="text-slate-800 dark:text-slate-100 underline decoration-2 underline-offset-4">{{ q.options[q.answer] }}</span></p>
                      <div class="bg-white/70 dark:bg-slate-800/70 p-4 md:p-5 rounded-xl border border-white/50 dark:border-slate-700 shadow-sm relative">
                        <span class="font-black text-medical-blue dark:text-blue-400 text-xs md:text-sm tracking-widest block mb-2"><i class="fa-solid fa-chalkboard-user mr-1.5"></i>解析</span>
                        <p class="text-slate-700 dark:text-slate-300 text-sm md:text-base leading-relaxed font-medium">{{ q.explanation }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <button v-if="!showExplanations" @click="checkAnswers" class="w-full py-5 md:py-6 bg-medical-blue dark:bg-blue-600 text-white rounded-3xl font-black text-xl md:text-2xl shadow-xl hover:bg-blue-800 dark:hover:bg-blue-700 hover:shadow-2xl active:scale-[0.98] transition-all flex justify-center items-center gap-3 border-2 border-blue-400 dark:border-blue-500">
