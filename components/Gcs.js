    ...
    <div class="bg-slate-50 dark:bg-slate-900 md:rounded-2xl shadow-sm border-x border-b md:border-t border-slate-200 dark:border-slate-700 relative overflow-hidden flex flex-col w-full h-full transition-colors">
      ...
      <div v-else class="flex flex-col flex-1 h-full relative overflow-hidden">
        ...
        <div ref="gcsContainer" class="p-3 md:p-5 flex-1 overflow-y-auto overscroll-none no-scrollbar w-full scroll-smooth">
          <div class="flex flex-col lg:grid lg:grid-cols-12 gap-4 md:gap-6 w-full lg:h-full min-h-max">
            
            <div class="lg:col-span-5 flex flex-col gap-3 md:gap-4 w-full lg:h-full shrink-0">
              ...
              <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-3 md:p-4 shrink-0">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                  <button v-for="act in fixedActions" :key="act.id" @click="doAction(act)" :disabled="showResult || (act.req && !actionProgress[act.req])" 
                    :class="['py-2 px-1 md:px-2 rounded-xl border-2 text-[11px] md:text-sm font-bold transition-all shadow-sm flex flex-col items-center justify-center gap-1 active:scale-95 text-center leading-snug', 
              ...
              <transition name="fade-slide" mode="out-in">
                <div :key="latestActionName" class="flex-1 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/40 dark:to-indigo-900/40 rounded-2xl shadow-md border-2 border-blue-200 dark:border-blue-800 p-4 md:p-6 flex flex-col items-center justify-center text-center min-h-[140px] relative overflow-hidden shrink-0">
                  ...
                  <div v-else class="w-full">
                    ...
                    <p class="text-base md:text-xl font-black text-slate-800 dark:text-slate-100 leading-snug">{{ latestReaction }}</p>
                  </div>
                </div>
              </transition>
            </div>

            <div class="lg:col-span-7 flex flex-col w-full lg:h-full relative">
              <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm rounded-2xl p-4 md:p-5 w-full flex flex-col flex-1 transition-colors relative">
                ...
                <div class="flex-1 relative flex flex-col">
                  <transition name="fade-slide" mode="out-in">
                    ...
                    <div v-if="!showResult" key="scoring" class="flex flex-col justify-between flex-1 gap-4">
                      <div class="space-y-3 md:space-y-4 flex-1">
                        ...
                        <div class="bg-slate-50 dark:bg-slate-900/50 p-2 md:p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                          ...
                          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            <button v-for="val in [4,3,2,1]" :key="'e'+val" @click="userE = val" 
                              ...
                              <span v-if="showHelperText" class="text-[11px] md:text-xs font-bold leading-tight mt-0.5 text-center px-1">{{ ['沒有反應', '疼痛刺激睜眼', '呼喚睜眼', '主動睜眼'][val-1] }}</span>
                            </button>
                          </div>
                        </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-900/50 p-2 md:p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                          ...
                          <div class="grid grid-cols-2 min-[400px]:grid-cols-3 sm:grid-cols-5 gap-2">
                            <button v-for="val in [5,4,3,2,1]" :key="'v'+val" @click="userV = val" 
                              ...
                              <span class="text-xl md:text-2xl font-black">{{ val }}</span>
                              <span v-if="showHelperText" class="text-[10px] md:text-[11px] font-bold leading-tight mt-0.5 text-center px-1">{{ ['沒有反應', '只能發出聲音', '只能說出單詞', '回答錯誤', '回答正確'][val-1] }}</span>
                            </button>
                          </div>
                        </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-900/50 p-2 md:p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                          ...
                          <div class="grid grid-cols-2 min-[400px]:grid-cols-3 sm:grid-cols-6 gap-2">
                            <button v-for="val in [6,5,4,3,2,1]" :key="'m'+val" @click="userM = val" 
                              ...
                              <span class="text-xl md:text-2xl font-black">{{ val }}</span>
                              <span v-if="showHelperText" class="text-[10px] md:text-[11px] font-bold leading-tight mt-0.5 text-center px-1">{{ ['沒有反應', '異常伸展', '痛刺激屈曲', '痛刺激僵直', '定位痛刺激', '聽從指示'][val-1] }}</span>
                            </button>
                          </div>
                        </div>
                      </div>
                    ...
                    <div v-else key="result" class="flex flex-col justify-between flex-1 gap-4">
                      <div class="bg-slate-50 dark:bg-slate-800/80 p-4 md:p-5 rounded-xl border-2 flex flex-col flex-1" :class="isCorrect ? 'border-green-400 dark:border-green-600' : 'border-red-400 dark:border-red-600'">
                        
                        <div class="flex items-center gap-3 mb-4 border-b border-slate-200 dark:border-slate-700 pb-4 shrink-0">
                          <i :class="['fa-solid text-3xl md:text-4xl', isCorrect ? 'fa-circle-check text-green-500' : 'fa-triangle-exclamation text-red-500']"></i>
                          <div>
                        ...
                        <div class="grid grid-cols-3 gap-1 md:gap-2 mb-4 shrink-0">
                          <div class="text-center p-1.5 md:p-2 rounded-xl border" :class="userE === currentCase.answer.e ? 'bg-green-100/50 border-green-200 dark:bg-green-900/30' : 'bg-red-100/50 border-red-200 dark:bg-red-900/30'">
                            <p class="text-[9px] md:text-[10px] font-bold text-slate-500 mb-0.5 truncate">E (睜眼)</p>
                            <p class="font-black text-sm md:text-lg" :class="userE === currentCase.answer.e ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">{{userE}} <span class="text-[9px] md:text-[10px] font-normal text-slate-400 block md:inline">/ 正解 {{currentCase.answer.e}}</span></p>
                          </div>
                          <div class="text-center p-1.5 md:p-2 rounded-xl border" :class="userV === currentCase.answer.v ? 'bg-green-100/50 border-green-200 dark:bg-green-900/30' : 'bg-red-100/50 border-red-200 dark:bg-red-900/30'">
                            <p class="text-[9px] md:text-[10px] font-bold text-slate-500 mb-0.5 truncate">V (語言)</p>
                            <p class="font-black text-sm md:text-lg" :class="userV === currentCase.answer.v ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">{{userV}} <span class="text-[9px] md:text-[10px] font-normal text-slate-400 block md:inline">/ 正解 {{currentCase.answer.v}}</span></p>
                          </div>
                          <div class="text-center p-1.5 md:p-2 rounded-xl border" :class="userM === currentCase.answer.m ? 'bg-green-100/50 border-green-200 dark:bg-green-900/30' : 'bg-red-100/50 border-red-200 dark:bg-red-900/30'">
                            <p class="text-[9px] md:text-[10px] font-bold text-slate-500 mb-0.5 truncate">M (運動)</p>
                            <p class="font-black text-sm md:text-lg" :class="userM === currentCase.answer.m ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">{{userM}} <span class="text-[9px] md:text-[10px] font-normal text-slate-400 block md:inline">/ 正解 {{currentCase.answer.m}}</span></p>
                          </div>
                        </div>

                        <div class="flex-1 bg-blue-50/50 dark:bg-slate-900/50 p-3 md:p-4 rounded-xl border border-blue-100 dark:border-slate-700">
                      ...
                      <button @click="nextCase" class="w-full py-3 bg-medical-blue dark:bg-blue-600 text-white rounded-xl font-black text-base md:text-lg shadow-md hover:bg-blue-800 dark:hover:bg-blue-700 transition-all active:scale-95 border-2 border-blue-400 shrink-0">
