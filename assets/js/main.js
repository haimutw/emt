...
createApp({
  components: { Home, Courses, Quiz, Gcs, Scenario },
  data() {
    return {
      currentView: 'Home',
      routeParam: null,
      ...
      userHistory: { courses: [], quiz: {}, gcs: {}, last_courses: null },
  ...
  methods: {
    ...
    handleHistoryUpdate(payload) {
      ...
      if (payload.type === 'courses') {
        if (!this.userHistory.courses.includes(payload.value)) this.userHistory.courses.push(payload.value);
      } else if (payload.type === 'last_courses') {
        this.userHistory.last_courses = payload.value;
      } else if (payload.type === 'quiz' || payload.type === 'gcs') {
    ...
    onHashChange() {
      ...
      const validViews = ['Home', 'Courses', 'Quiz', 'Gcs', 'Scenario'];
